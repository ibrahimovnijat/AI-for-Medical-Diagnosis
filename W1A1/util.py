...
def get_mean_std_per_batch(image_dir, df, H=320, W=320):
    ...
    for img in df.sample(100)["Image"].values:
        ...
        # img_ = ImageOps.grayscale(img_)
        # img_ = img_.resize((H,W), Image.NEAREST)
        # sample_data.append(np.array(img_, dtype=np.float))
        
        # sample_data.append(np.array(image.load_img(image_path, target_size=(H,W))))
        sample_data.append(np.array(keras.utils.load_img(image_path, target_size=(H,W))))
...
def load_image(img, image_dir, df, preprocess=True, H=320, W=320):
    ...
    # img_ = ImageOps.grayscale(img_)
    # img_ = img_.resize((H,W), Image.NEAREST)
    # x = np.array(img_, dtype=np.float)
